...
/**
 * For more documentation, see the class DT_Demo_Data_Migration in file
 * dt-core/migrations/abstract.php
 */
class DT_Demo_Data_Migration_Engine
{
        ...
        foreach ( $filenames as $filename ) {
            if ( $filename[0] !== '.' && $filename !== 'abstract.php' ){
                if ( preg_match( '/^([0-9][0-9][0-9][0-9])(-.*)?\.php$/i', $filename, $matches ) ) {
                    ...
                    $migration_name = sprintf( 'DT_Demo_Data_Migration_%04d', $got_migration_number );
     ...
     *
     * @throws \Exception ...
     * @throws \DT_Demo_Data_Migration_Lock_Exception ...
     * @throws \Throwable ...
     */
    public static function migrate( int $target_migration_number ) {
        ...
        while ( true ) {
            $current_migration_number = get_option( 'dt_demo_data_migration_number' );
            ...
            if ( (int) get_option( 'dt_demo_data_migration_lock', 0 ) ) {
                throw new DT_Demo_Data_Migration_Lock_Exception();
            }
            update_option( 'dt_demo_data_migration_lock', '1' );

            error_log( gmdate( ' Y-m-d H:i:s T' ) . " Starting migrating Demo Data to number $activating_migration_number" );
            try {
                $migration->up();
            } catch ( Throwable $e ) {
                update_option( 'dt_demo_data_migrate_last_error',
                    array(
            ...
            update_option( 'dt_demo_data_migration_number', (string) $activating_migration_number );
            error_log( gmdate( ' Y-m-d H:i:s T' ) . " Done migrating Demo Data to number $activating_migration_number" );

            update_option( 'dt_demo_data_migration_lock', '0' );
...
//            }
    ...
    public static function get_current_db_migration(){
        return get_option( 'dt_demo_data_migration_number', 0 );
    }
    public static function display_migration_and_lock(){
        add_action( 'dt_utilities_system_details', function () {
            $lock = get_option( 'dt_demo_data_migration_lock', 0 ); ?>
            <tr>
                <td><?php echo esc_html( sprintf( __( 'Demo Data migration version: %1$s of %2$s' ), self::get_current_db_migration(), self::$migration_number ) ); ?>. Lock: <?php echo esc_html( $lock ); ?>  </td>
                <td> <button name="reset_lock" value="dt_demo_data_migration_lock">Reset Lock</button></td>
            </tr>
        <?php });
    }
}
...
class DT_Demo_Data_Migration_Lock_Exception extends Exception
{
    public function __construct( $message = null, $code = 0, Exception $previous = null ) {
         ...
         */
        $last_migration_error = get_option( 'dt_demo_data_migrate_last_error' );
        if ( $message === null ) {
